...
excludes chance from below. The consequences propagate downstream, where a
...
\item A measured predictability asymmetry across five candidate targets on
196 equities and ten years, showing that second-moment quantities are
...
\subsection{Financial Language Models and Evaluation}
Domain-adapted models such as FinBERT~\cite{araci2019finbert} improve
...
Evaluation practice has received less attention than modelling. Sullivan and
colleagues established that apparent profitability shrinks once the search
that produced it is priced in, a concern that transfers directly to
comparisons among agent architectures. Our evaluation accordingly reports
interval estimates and hypothesis tests, and states the sample size each
class of claim would require.
...
shifting conditions~\cite{mantshimuli2026toward}. The Risk Manager described
here is deliberately transparent: it maps estimated uncertainty and a stated
tolerance onto a position size, without solving a portfolio optimisation.
...
required. The orchestrator dispatches exactly those subtasks. A request for a
definition dispatches nothing; a request bearing on the user's own capital
always dispatches the Risk Manager, whether or not the user asked about
downside.
...
the synthesis stage can reweight over what remains instead of treating an
absence as evidence.
...
and a confidence rather than a directional label.}
\label{fig:agents}
\end{figure}
...
inferred risk posture from how a user describes loss tolerance rather than
from an explicit label alone.

Two post-conditions are enforced after classification rather than requested
of the model, since a prompt expresses an intention and a post-condition
...
Researcher is therefore evaluated prospectively, on live operation, and is
excluded from the historical results in Section~\ref{sec:evaluation}.
...
that design at scale, rather than on a handful of names, produced the result
in Section~\ref{sec:predictability}: the rank correlation between predicted
and realised direction is negative at every horizon tested. A point estimate
of an unforecastable quantity is not a useful object, and every mechanism
that scales exposure by such an estimate scales it by noise.
...
estimate is shrunk hard toward zero, capped at a small fraction of the
...
A raw confidence is formed from $|\hat{\mu}_h/\hat{\sigma}_h|$ and then
mapped through a reliability curve fitted on realised outcomes, so a stated
value corresponds to an observed frequency rather than to model self-report.
When the ratio falls below a threshold the Analyst abstains, returning no
view rather than a weak one. Abstention is treated as a first-class output:
given the evidence in Section~\ref{sec:predictability}, no view is frequently
the correct answer, and a system unable to express it will manufacture one.
...
beforehand, and Section~\ref{sec:power} quantifies what that error is worth.
...
and are scored only on subsequent unseen observations. The universe was
selected at the end of the sample, so every constituent survived the period;
this survivorship flatters all reported columns and applies equally to the
system and its baselines.

\subsection{Predictability of Candidate Targets}
\label{sec:predictability}
Table~\ref{tab:predict} reports, for each candidate target, the rank
correlation between a forecast built from trailing information and the
realised outcome over the subsequent 21 trading days.

\begin{table}[!ht]
\centering
\setlength{\belowcaptionskip}{3pt}
\caption{Predictability of candidate targets. 196 equities, ten years,
43{,}512 out-of-sample observations, 21-day horizon.}
\vspace{-2.5mm}
\begin{tabular}{
  >{\arraybackslash}m{95pt}
...
directional relationship carries the wrong sign. Table~\ref{tab:analyst}
confirms this on the deployed Analyst across three horizons.

\begin{table}[!ht]
\centering
\setlength{\belowcaptionskip}{3pt}
\caption{Analyst directional performance, 39 equities, 8{,}814 out-of-sample
decisions. Intervals are 95\% Wilson bounds.}
\vspace{-2.5mm}
\begin{tabular}{
  ...
  \toprule
  \textbf{Horizon} & \textbf{Accuracy} & \textbf{95\% CI} & \textbf{Rank corr.} \\
  \midrule
...
At one day the interval lies wholly below $0.50$: the forecast is
significantly worse than chance, not merely uninformative. At longer horizons
it is indistinguishable from chance. This is the empirical basis for the
redesign in Section~\ref{sec:analyst}.

Volatility forecasting was assessed on the same protocol using QLIKE, which
remains valid when the target is observed through a noisy proxy. Over 5{,}700
forecasts the heterogeneous autoregressive model attains $-6.789$ against
$-6.725$ for a random walk, confirming genuine persistence; an exponentially
weighted estimator reaches $-6.823$, so the simplest conditional baseline is
not displaced. Volatility ordering is estimated well even where its level is
not, which is what a sizing rule requires.

\subsection{Confidence-Aware Abstention}
Table~\ref{tab:abstain} reports accuracy as a function of coverage, where
coverage is the proportion of queries on which the system commits to a view
and abstention removes the least confident.
...
confidence ranks its own reliability: it knows when it does not know. This is
the clearest positive result we obtain, and it is available precisely because
each agent is evaluated on its own output rather than through a portfolio
aggregate.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\linewidth]{coverage_accuracy.png}
\caption{Accuracy against coverage under confidence-based abstention, with
95\% Wilson intervals. The dashed line marks chance. Withholding the least
confident decisions raises accuracy monotonically.}
\label{fig:coverage}
\end{figure}

\subsection{Downstream Consequences for Position Sizing}
Conditioning exposure on an uninformative forecast has a measurable cost. We
compare the sized policy against a control holding the system's own average
exposure every day with no timing whatsoever, which separates genuine risk
management from simply holding less. The sized policy attains a Sharpe ratio
of $0.381$, an annualised return of $9.97\%$ and a maximum drawdown of
$-38.0\%$; the constant-exposure control at the same average exposure attains
$0.539$, $11.52\%$ and $-33.8\%$ respectively, dominating on all three. A
fully invested arm reproduces buy-and-hold exactly, confirming the harness.
...
The methodological point generalises: an evaluation comparing only against
buy-and-hold cannot detect this failure, because reduced exposure lowers
drawdown and thereby presents as risk management. A matched-exposure control
should accompany any such comparison.

\subsection{Per-Agent Measurements}
The Planner classifies intent at $0.692$ on labels published with the source
study, against $0.75$ for a supervised classifier trained on that corpus;
we report this as a component measurement rather than as a contribution.
The Researcher is excluded from historical evaluation for the reasons given
in Section~\ref{sec:agents}. Derived drawdown probabilities improve on the
unconditional base rate only marginally, and their reliability curve slopes
downward, overstating risk where predicted risk is highest; the aggregate
calibration error conceals this because the per-bin errors carry opposite
signs.

\subsection{Data Requirements}
\label{sec:power}
Reporting what each class of claim would require clarifies which conclusions
are settled. With a strategy-to-benchmark correlation near $0.55$, separating
a Sharpe improvement of $+0.30$ at conventional power needs approximately
19{,}692 trading days; the available holdout supports $5.9\%$ power against
a $5\%$ false-positive rate. Breadth does not resolve this, since
...
and calibration, by contrast, converge within hundreds of observations, which
is why they carry the conclusions here.

The same analysis explains why execution timing matters. Applying a decision
to the session whose close produced it inflated mean Sharpe from $0.192$ to
$0.724$ in an earlier revision of this system and reversed its reported
outcome. The defect presented as skill because the comparison baseline was
correctly lagged; regression tests now guard against its return.
...
inputs. This asymmetry is actionable rather than merely descriptive: an
...
permitting historical evaluation of the Researcher, and report the live
